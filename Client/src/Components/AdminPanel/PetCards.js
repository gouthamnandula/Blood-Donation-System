...
const PetCards = (props) => {
  ...
  const handleApprove = async () => {
    setIsApproving(true);
    try {
      const response = await fetch(`http://51.20.183.104:4000/approving/${props.pet._id}`, {
        method: 'PUT',
        body: JSON.stringify({
          status: "Approved"
        }),
  ...
  const deleteFormsAdoptedPet = async () => {
    setIsDeleting(true)
    try {
      const deleteResponses = await fetch(`http://51.20.183.104:4000/form/delete/many/${props.pet._id}`, {
        method: 'DELETE'
      });
  ...
  const handleReject = async () => {
    try {
      const response = await fetch(`http://51.20.183.104:4000/delete/${props.pet._id}`, {
        method: 'DELETE'
      })
