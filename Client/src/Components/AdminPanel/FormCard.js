...
const FormCard = (props) => {
  ...
  const handleApprove = async () => {
    setIsApproving(true);
    try {
      const response = await fetch(`http://51.20.183.104:4000/approving/${props.form.petId}`, {
        method: 'PUT',
        body: JSON.stringify({
  ...
  const deleteFormAdoptedPet = async () => {
    try {
      const deleteResponse = await fetch(`http://51.20.183.104:4000/form/delete/many/${props.form.petId}`, {
        method: 'DELETE'
      });
  ...
  const handleReject = async () => {
    setIsDeleting(true)
    try {
      const response = await fetch(`http://51.20.183.104:4000/form/reject/${props.form._id}`, {
        method: 'DELETE'
      })
